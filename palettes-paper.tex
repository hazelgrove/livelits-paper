...
\item \li{Type1} ...\li{Typen} are valid types
\item \li{ExpansionType} is a valid type
\item \li{D : TD} for some type \li{TD}
\item and \li{P} identifies a closed module that can be loaded from package \li{pkg} such that \begin{lstlisting}
P : IPalette
    with type Arguments = { 
      arg1 : HoleRef<Type1>, 
      ..., 
    ...
    with type ExpansionType = ExpansionType
    with type Dependencies = TD
\end{lstlisting}
where \li{IPalette} is defined in Fig.~\ref{fig:IPalette}.
\end{enumerate}
...
module type IPalette = {
  ...
  val init_model : Arguments -> 
                   MHoleRef(Model)
  val update : (Model, Msg) -> 
               MHoleRef(Model)
...
type HoleRef('a)
type MHoleRef('a)
val fresh_hole : Type<Dependencies |- *> -> MHoleRef(HoleRef('a))
val bind : MHoleRef('a) -> 
           ('a -> MHoleRef('b)) -> 
           MHoleRef('b)
val return : 'a -> MHoleRef('a)             
\end{lstlisting}
\caption{Signature for working with hole refs}
\label{fig:HoleRefs}
\end{figure}
